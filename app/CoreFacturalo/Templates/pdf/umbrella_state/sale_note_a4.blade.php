...
<table class="full-width mt-5">
    ...
    <tr>
        ...
        <td>{{ $customer->number }}</td>
            ...
            <td>{{ $document->purchase_order? $document->purchase_order : ''}}</td>
    </tr>
    ...
    <tr>
        ...
        <td width="15%">{{ $customer->telephone!== ''?$customer->telephone :'' }}</td>
        @endif
        @if($document->user)
            <td width="25%">Asesor de ventas:</td>
            <td width="15%">{{ $document->user->name }}</td>
        @endif
    </tr>
    ...
    {{-- <tr>
        <td></td>
        @if($document->user)
        <td class="align-top">Asesor de ventas:</td>
        <td colspan="3">{{ $document->user->name }}</td>
        @endif
    </tr> --}}
...
<tr>
    ...
    <strong>Pagos:</strong> </td></tr>
        ...
        @foreach($payments as $row)
            <tr>
                <td>- {{ $row->date_of_payment->format('d/m/Y') }} {{ $document->currency_type->symbol }} {{ $row->payment }} </td>
                <td width="35%"  style="text-transform: uppercase;">{{$row->payment_method_type->description}}</td>
                @if ($row->has_card)
                    <td width="35%" style="text-transform: uppercase;">{{$row->card_brand->description}}</td>
                @endif
