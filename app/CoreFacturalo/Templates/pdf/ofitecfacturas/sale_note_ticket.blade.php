...
<table class="full-width">
    ...
    <tr>
        <td><p class="desc">Nro. Teléfono:</p></td>
...
<table class="full-width mt-10 mb-10">
    ...
    @foreach($document->items as $row)
        <tr>
            ...
            <td class="text-right desc-9 align-top">{{ number_format($row->unit_price, 0,",",".") }}</td>
            <td class="text-right desc-9 align-top">{{ number_format($row->total, 0,",",".") }}</td>
        </tr>
        <tr>
    ...
    @endforeach
        @if($document->total_exportation > 0)
            <tr>
                ...
                <td class="text-right font-bold desc">{{ number_format($document->total_exportation, 0,",",".") }}</td>
            </tr>
        @endif
        @if($document->total_free > 0)
            <tr>
                <td colspan="4" class="text-right font-bold desc">OP. GRATUITAS: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold desc">{{ number_format($document->total_free, 0,",",".") }}</td>
            </tr>
        @endif
        @if($document->total_unaffected > 0)
            <tr>
                <td colspan="4" class="text-right font-bold desc">OP. INAFECTAS: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold desc">{{ number_format($document->total_unaffected, 0,",",".") }}</td>
            </tr>
        @endif
        @if($document->total_exonerated > 0)
            <tr>
                <td colspan="4" class="text-right font-bold desc">OP. EXONERADAS: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold desc">{{ number_format($document->total_exonerated, 0,",",".") }}</td>
            </tr>
        @endif
        ...
        @endif --}}
         @if($document->total_discount > 0)
            <tr>
                ...
                <td class="text-right font-bold desc">{{ number_format($document->total_discount, 0,",",".") }}</td>
            </tr>
        @endif
        {{--<tr>
        ...
        @if($document->total_charge > 0 && $document->charges)
            <tr>
                ...
                <td class="text-right font-bold desc">{{ number_format($document->total_charge, 0,",",".") }}</td>
            </tr>
        @endif
        
        <tr>
            <td colspan="4" class="text-right font-bold desc">TOTAL A PAGAR: {{ $document->currency_type->symbol }}</td>
            <td class="text-right font-bold desc">{{ number_format($document->total, 0,",",".") }}</td>
        </tr>
        ...
        @if($change_payment < 0)
            <tr>
                ...
                <td class="text-right font-bold desc">{{ number_format(abs($change_payment),0, ",", ".") }}</td>
            </tr>
        @endif
