...
class InventoryValuedKardex
{
    ...
    public static function getDataFormatSunat($params, $init = [])
    {
        ...
        $all_record_items = ($purchase_items)->merge($document_items);
        $initial_stock = InventoryKardex::where('item_id', $item->id)->where('inventory_kardexable_type', ModelsInventory::class)
            ->whereBetween('date_of_issue', [$params->date_start, $params->date_end]);
            if($params->establishment_id){
                $initial_stock->where('warehouse_id', $params->establishment_id);
            }
            $initial_stock = $initial_stock->first();
            
            
        if ($initial_stock) {
            $inventory = ModelsInventory::find($initial_stock->inventory_kardexable_id);
            if ($inventory->description == "Stock inicial") {
                //add to initial_stock property model_type with value "inventory"
                $initial_stock->model_type = "inventory";
                $all_record_items = (collect([$initial_stock]))->merge($all_record_items);
            }
        }
        return [
            'item' => $item,
            'records' => self::getRecordsFromItems($all_record_items, $init)
        ];
    }
    ...
    private static function getRecordsFromItems($collection, $init = [])
    {
        ...
        $balance_unit_cost = 0;
        if (count($init) > 0) {
            $balance_quantity = $init["stock"] ?? 0;
            $balance_unit_cost = $init["cost_unit"] ?? 0;
            $balance_total_cost = $init["cost_total"] ?? 0;
        }
        foreach ($new_collection as $key => $temp_data) {
            ...
            if ($key == 0) {
                ...
                $balance_unit_cost = ($balance_quantity != 0) ? $unit_cost : null;
                ...
                $balance_total_cost += ($temp_data['type'] == 'input') ? $temp_data['total'] * $temp_data['factor'] : $temp_data['output_quantity'] * $cost_average * $temp_data['factor'];
            ...
            if ($temp_data['type'] == 'output') {
                $temp_data['output_unit_price'] = $unit_cost;
                $temp_data['output_total'] = $temp_data['output_quantity'] * $unit_cost;
            }
    ...
    {
        $class_ = get_class($record_item); //DocumentItem, PurchaseItem, DispatchItem, ModelsInventory
        $temp_data = [];
        ...
        } else if ($record_item instanceof InventoryKardex) {
            $document = $record_item;
            $purchase_price = Item::find($document->item_id)->purchase_unit_price;        
            $temp_data = [
                'id' => $document->id,
                'type' => 'input',
                ...
                'input_quantity' => $record_item->quantity,
                'input_unit_price' => $purchase_price,
                'input_total' => $record_item->quantity * $purchase_price,
                ...
                'factor' => 1,
                'quantity' => $record_item->quantity,
                'total' => $record_item->quantity * $purchase_price,
