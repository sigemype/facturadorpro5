...
class IncomeResource extends JsonResource
{
    ...
    {
        $income = Income::with(['items'])->find($this->id);
        $income->payments = self::getTransformPayments($income->payments);
        return [
            'id' => $this->id,
    ...
    public static function getTransformPayments($payments){
        ...
        return $payments->transform(function($row, $key){ 
            return [
                ...
                // 'payment_destination_disabled' => ($row->payment_method_type_id == 1) ? true:false 
            ];
        }); 
